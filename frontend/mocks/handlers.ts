...
export const handlers = [
  rest.post('/login', (req, res, ctx) => {
    ...
    if (password === 'password') {
      return res(ctx.cookie('remember_token', username), ctx.json({ message: 'success login' }))
    } else {
      return res(ctx.status(400), ctx.json({ message: 'Failed to login' }))
    }
  }),
  ...
  rest.get('/questions/latest', (req, res, ctx) => res(ctx.json(latestNovels))),
